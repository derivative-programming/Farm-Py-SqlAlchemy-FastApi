...
class MockFlowPacUserTacListInitReportResult:
    """
    A mock object for the FlowPacUserTacListInitReportResult class.
    """
    def __init__(self):
        """
...
def flow_response():
    """
    Return a mock FlowPacUserTacListInitReportResult object.
    """
...
async def test_process_request(flow_response):
    ...
    mock_pac_bus_obj = patch(
        ...
        autospec=True).start()
    mock_flow = patch(
        'apis.models.init.pac_user_tac_list_init_report.FlowPacUserTacListInitReport',
        autospec=True).start()
