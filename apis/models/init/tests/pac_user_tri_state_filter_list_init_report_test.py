...
async def test_process_request(flow_response):
    ...
    mock_pac_bus_obj = patch(
        'apis.models.init.pac_user_tri_state_filter_list_init_report.PacBusObj',
        autospec=True).start()
    mock_flow = patch(
        'apis.models.init.pac_user_tri_state_filter_list_init_report.FlowPacUserTriStateFilterListInitReport',
        autospec=True).start()
