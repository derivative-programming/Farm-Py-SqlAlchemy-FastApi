# apis/models/init/tests/pac_user_flavor_list_init_report_test.py
"""
...
async def test_process_request(flow_response):
    ...
    mock_pac_bus_obj = patch(
        'apis.models.init.pac_user_flavor_list_init_report.PacBusObj',
        autospec=True).start()
    mock_flow = patch(
        'apis.models.init.pac_user_flavor_list_init_report.FlowPacUserFlavorListInitReport',
        autospec=True).start()
    mock_flow_instance = mock_flow.return_value
    mock_flow_instance.process = AsyncMock(return_value=flow_response)
    request = PacUserFlavorListInitReportGetInitModelRequest()
    response = PacUserFlavorListInitReportGetInitModelResponse()
    pac_code = uuid.uuid4()
