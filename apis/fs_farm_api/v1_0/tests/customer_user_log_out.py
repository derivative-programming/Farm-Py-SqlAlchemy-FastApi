...
from apis.factories import CustomerUserLogOutPostModelRequestFactory
from helpers import ApiToken
from models import CurrentRuntime
import logging

class CustomerUserLogOutViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        api_dict = {'CustomerCode': str(self.customer.code), 'role_name_csv':'User'}
    ...
    def test_submit_failure2(self):
        response = self.client.get('/api/v1_0/customer-user-log-out/xxx/', **self.valid_header)
        self.assertEqual(response.status_code, 404)

    def test_submit_failure3(self):
        response = self.client.get('/api/v1_0/customer-user-log-out/', **self.valid_header)
        self.assertEqual(response.status_code, 501)

    def test_init_success(self):
        response = self.client.get(f'/api/v1_0/customer-user-log-out/{self.customer.code}/init/', **self.valid_header)
        self.assertEqual(response.status_code, 200)
        json_string = response.content.decode()
        responseDict = json.loads(json_string)
        self.assertTrue(response.data['success'])
    ...
    def test_init_failure(self):
        response = self.client.get('/api/v1_0/customer-user-log-out/xxx/init/', **self.valid_header)
        self.assertEqual(response.status_code, 404)

    def test_init_failure2(self):
        response = self.client.get('/api/v1_0/customer-user-log-out/init/', **self.valid_header)
        self.assertEqual(response.status_code, 404)
