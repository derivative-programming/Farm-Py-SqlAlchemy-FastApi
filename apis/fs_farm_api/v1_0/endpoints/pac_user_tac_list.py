# apis/fs_farm_api/v1_0/endpoints/pac_user_tac_list.py  # pylint: disable=duplicate-code
...
class PacUserTacListRouterConfig():  # pylint: disable=too-few-public-methods
    """
...
class PacUserTacListRouter(BaseRouter):
    ...
    async def request_get_with_id(
        pac_code: uuid.UUID = Path(..., description=PAC_CODE),
        request_model:
            api_models.PacUserTacListGetModelRequest = (
                Depends()),
        session: AsyncSession = Depends(get_db),
        api_key: str = Depends(api_key_header)
    ):
    ...
    async def request_get_with_id_to_csv(
        ...
        async with session:
            try:
                ...
                report_manager = \
                    reports.ReportManagerPacUserTacList(
                        session_context)
