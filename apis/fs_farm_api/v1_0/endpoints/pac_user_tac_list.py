...
class PacUserTacListRouter(BaseRouter):
    ...
    @staticmethod
    @router.get("/api/v1_0/pac-user-tac-list/{pac_code}/init",
                response_model=api_init_models.PacUserTacListInitReportGetInitModelResponse,
    ...
    async def request_get_init(pac_code: str = Path(..., description="Pac Code"),
        ...
        logging.info('PacUserTacListRouter.init get result:%s',response_data)
        return response

    @staticmethod
    @router.get("/api/v1_0/pac-user-tac-list/{pac_code}",
        ...
        async with session:
            try:
                ...
                if response.success is True:
                    ...
                    await session.rollback()
        logging.info('PacUserTacListRouter.submit get result:$s', response.model_dump_json())
        return response

    @staticmethod
    @router.get("/api/v1_0/pac-user-tac-list/{pac_code}/to-csv",
    ...
    async def request_get_with_id_to_csv(pac_code: str = Path(..., description="Pac Code"),
        ...
        async with session:
            try:
                ...
                report_manager = reports.ReportManagerPacUserTacList(session_context)
                ...
                if response.success is True:
                    ...
                    await session.rollback()
        logging.info('PacUserTacListRouter.submit get result:$s', response.model_dump_json())
        output_file_name = 'pac_user_tac_list_' + pac_code + '_' + str(uuid.uuid4()) + '.csv'
