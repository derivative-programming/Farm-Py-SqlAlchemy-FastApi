...
class PacUserRoleListRouterConfig():
    """
    Configuration class for the PacUserRoleListRouter.
    """
    # constants
...
class PacUserRoleListRouter(BaseRouter):
    ...
    async def request_get_init(
        ...
        logging.info('PacUserRoleListRouter.init get result:%s',
                     response_data)
        return response
    ...
    async def request_get_with_id(
        ...
        response_data = response.model_dump_json()
        logging.info(
            "PacUserRoleListRouter.submit get result:%s",
            response_data
        )
        return response
    ...
    async def request_get_with_id_to_csv(
        ...
        async with session:
            try:
                ...
                report_manager = reports.ReportManagerPacUserRoleList(
                    session_context)
        ...
        output_file_name = (
            f'pac_user_role_list_{str(pac_code)}_{str(uuid_value)}.csv'
        )
        return FileResponse(
            tmp_file_path,
            media_type='text/csv',
