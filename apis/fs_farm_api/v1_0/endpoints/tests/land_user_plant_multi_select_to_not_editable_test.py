...
"""
This module contains unit tests for the `land_user_plant_multi_select_to_not_editable` endpoint.
"""
import logging
import uuid
...
async def test_submit_success(overridden_get_db):
    ...
    2. Patch the `process_request` method of
        `LandUserPlantMultiSelectToNotEditablePostModelResponse` with the mock function.
    3. Create a land using the `LandFactory`.
    4. Generate an API key for the land.
    5. Send a POST request to the `land-user-plant-multi-select-to-not-editable`
        endpoint with the land code and API key.
...
async def test_submit_request_validation_error(overridden_get_db):
    ...
    3. Send a POST request to the `land-user-plant-multi-select-to-not-editable`
    endpoint with the land code, invalid data, and API key.
...
async def test_submit_authorization_failure_empty_header_key(
    ...
    '/api/v1_0/land-user-plant-multi-select-to-not-editable/{land_code}' endpoint
...
async def test_submit_endpoint_url_failure(
    ...
    This test case sends a POST request to the
    '/api/v1_0/land-user-plant-multi-select-to-not-editable/{land_code}/xxxx' endpoint
...
async def test_submit_endpoint_invalid_code_failure(
    ...
    """
    Test case for the submit endpoint when an invalid land code is provided.
    This test case verifies that when an invalid land code is provided,
...
async def test_submit_endpoint_method_failure(
    ...
    '/api/v1_0/land-user-plant-multi-select-to-not-editable/{land_code}' endpoint with the API key in the headers. The expected
