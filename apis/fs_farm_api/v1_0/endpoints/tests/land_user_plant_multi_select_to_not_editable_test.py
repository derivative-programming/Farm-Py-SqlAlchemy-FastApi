...
from .....models.factory.land_user_plant_multi_select_to_not_editable  import LandUserPlantMultiSelectToNotEditablePostModelRequestFactory
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/land-user-plant-multi-select-to-not-editable/{.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
#             '/api/v1_0/land-user-plant-multi-select-to-not-editable/xxx/',
#             headers=valid_header
#         )
...
async def test_init_success(overridden_get_db: AsyncSession):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        response = await ac.get(
            f'/api/v1_0/land-user-plant-multi-select-to-not-editable/{land_code}/init',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/land-user-plant-multi-select-to-not-editable/{land_code}/initxxx/'
        )
        assert response.status_code == 501
