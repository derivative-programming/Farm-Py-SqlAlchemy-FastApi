...
from .....models.factory.customer_build_temp_api_key  import CustomerBuildTempApiKeyPostModelRequestFactory
...
#     api_dict = {'CustomerCode': str(.code), 'role_name_csv': 'User'}
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/customer-build-temp-api-key/{.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
#             '/api/v1_0/customer-build-temp-api-key/xxx/',
#             headers=valid_header
#         )
...
async def test_init_success(overridden_get_db: AsyncSession):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        response = await ac.get(
            f'/api/v1_0/customer-build-temp-api-key/{customer_code}/init',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/customer-build-temp-api-key/{customer_code}/initxxx/'
        )
        assert response.status_code == 501
