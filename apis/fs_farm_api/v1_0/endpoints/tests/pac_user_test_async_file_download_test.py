# apis/fs_farm_api/v1_0/endpoints/tests/pac_user_test_async_file_download_test.py  # pylint: disable=duplicate-code # noqa: E501
# pylint: disable=unused-import, too-many-public-methods

"""
This module contains unit tests for the
`pac_user_test_async_file_download` endpoint.
"""
...
async def test_submit_success(overridden_get_db):
    ...
    with patch.object(
        apis_models.PacUserTestAsyncFileDownloadPostModelResponse,
        'process_request',
...
async def test_submit_authorization_failure_bad_api_key(
    ...
    2. Get the pac code.
    3. Send a POST request to the
        '/api/v1_0/pac-user-test-async-file-download/{pac_code}'
        endpoint with an empty JSON payload and a bad API key.
    4. Verify the response status code based on the
        configuration of the PacUserTestAsyncFileDownloadRouter.

    If the PacUserTestAsyncFileDownloadRouterConfig.is_public is True,
    ...
    async with AsyncClient(
        ...
        if PacUserTestAsyncFileDownloadRouterConfig.is_public is True:
            assert response.status_code == 200
        else:
...
async def test_submit_endpoint_url_failure(
    ...
    This test case sends a POST request to the
    '/api/v1_0/pac-user-test-async-file-download/{pac_code}/xxxx'
    endpoint
