...
from .....models.factory.land_user_plant_multi_select_to_editable  import LandUserPlantMultiSelectToEditablePostModelRequestFactory
...
#     api_dict = {'LandCode': str(.code), 'role_name_csv': 'User'}
...
#             '/api/v1_0/land-user-plant-multi-select-to-editable/',
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/land-user-plant-multi-select-to-editable/{.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
#             '/api/v1_0/land-user-plant-multi-select-to-editable/xxx/',
#             headers=valid_header
#         )
...
async def test_init_success(overridden_get_db: AsyncSession):
    ...
    api_dict = {'LandCode': str(land_code), 'role_name_csv': 'User'}
    test_api_key = ApiToken.create_token(api_dict, 1)
    async with AsyncClient(app=app, base_url="http://test") as ac:

        app.dependency_overrides[get_db] = lambda: overridden_get_db
        response = await ac.get(
            f'/api/v1_0/land-user-plant-multi-select-to-editable/{land_code}/init',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/land-user-plant-multi-select-to-editable/{land_code}/initxxx/'
        )
        assert response.status_code == 501
