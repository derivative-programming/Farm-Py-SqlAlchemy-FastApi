...
# @pytest.fixture(scope="module")
# def valid_header(land):
#     # Replace with your actual token generation logic
#     api_dict = {'LandCode': str(land.code), 'role_name_csv': 'User'}
...
# @pytest.mark.asyncio
# async def test_submit_failure(overridden_get_db, invalid_request_data, valid_header, land):
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/land-add-plant/{land.code}/',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/land-add-plant/{land_code}/initxxx/' 
        )
        assert response.status_code == 501
