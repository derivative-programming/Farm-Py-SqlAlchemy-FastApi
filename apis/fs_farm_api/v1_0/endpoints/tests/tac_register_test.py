...
async def test_init_authorization_failure_bad_api_key(overridden_get_db: AsyncSession):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        if TacRegisterRouterConfig.is_public == True:
            assert response.status_code == 200
        else:
...
async def test_submit_success(overridden_get_db):
    async def mock_process_request(session, session_context, tac_code, request):
            pass
    with patch.object(apis_models.TacRegisterPostModelResponse, 'process_request', new_callable=AsyncMock) as mock_method:
