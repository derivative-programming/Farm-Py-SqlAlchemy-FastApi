...
from .....models.factory.error_log_config_resolve_error_log  import ErrorLogConfigResolveErrorLogPostModelRequestFactory
...
#     api_dict = {'ErrorLogCode': str(.code), 'role_name_csv': 'User'}
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/error-log-config-resolve-error-log/{.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
#             '/api/v1_0/error-log-config-resolve-error-log/xxx/',
#             headers=valid_header
#         )
...
async def test_init_success(overridden_get_db: AsyncSession):
    ...
    api_dict = {'ErrorLogCode': str(error_log_code), 'role_name_csv': 'User'}
    test_api_key = ApiToken.create_token(api_dict, 1)
    async with AsyncClient(app=app, base_url="http://test") as ac:

        app.dependency_overrides[get_db] = lambda: overridden_get_db
        response = await ac.get(
            f'/api/v1_0/error-log-config-resolve-error-log/{error_log_code}/init',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/error-log-config-resolve-error-log/{error_log_code}/initxxx/'
        )
        assert response.status_code == 501
