# apis/fs_farm_api/v1_0/endpoints/tests/pac_user_role_list_test.py
...
async def test_init_authorization_failure_bad_api_key(
    ...
    async with AsyncClient(
        ...
        if PacUserRoleListRouterConfig.is_public is True:
            assert response.status_code == 200
        else:
...
async def test_get_success(
    ...
    with patch.object(
        apis_models.PacUserRoleListGetModelResponse,
        'process_request',
...
async def test_get_authorization_failure_no_header(
    ...
    '/api/v1_0/pac-user-role-list/{pac_code}' endpoint
...
async def test_get_endpoint_url_failure(
    ...
    pac_code = pac.code
    request = await (
        request_factory.PacUserRoleListGetModelRequestFactory.
        create_async(
            overridden_get_db
        )
    )
...
async def test_get_csv_success(
    ...
    with patch.object(
        ...
        async with AsyncClient(
            ...
            app.dependency_overrides[get_db] = lambda: overridden_get_db
            response = await ac.get(
                f'/api/v1_0/pac-user-role-list/{pac_code}/to-csv',
                params=request_dict,
                headers={'API_KEY': test_api_key}
            )
...
async def test_get_csv_authorization_failure_bad_api_key(
    ...
    This test case sends a GET request to the
    '/api/v1_0/pac-user-role-list/{pac_code}/to-csv' endpoint
    ...
    2. Create a PacUserRoleListGetModelRequest using
        the PacUserRoleListGetModelRequestFactory.
...
async def test_get_csv_authorization_failure_empty_header_key(
    ...
    Steps:
    1. Create a test pac using the PacFactory.
    2. Create a PacUserRoleListGetModelRequest using the request_factory.
    ...
    If the PacUserRoleListRouterConfig.is_public is True:
    - The response status code should be 200.
    - The response content type should start with the test_constants.
        REPORT_TO_CSV_MEDIA_TYPE.

    If the PacUserRoleListRouterConfig.is_public is False:
...
async def test_get_csv_endpoint_url_failure(
    ...
    This test case verifies that the API endpoint
        '/api/v1_0/pac-user-role-list/{pac_code}/to-csv/xxx'
    ...
    async with AsyncClient(
        ...
        app.dependency_overrides[get_db] = lambda: overridden_get_db
        response = await ac.get(
            f'/api/v1_0/pac-user-role-list/{pac_code}/to-csv/xxx',
            headers={'API_KEY': test_api_key}
        )
...
async def test_get_csv_endpoint_invalid_code_failure(
    ...
    2. Create a request object using the PacUserRoleListGetModelRequestFactory.
    3. Convert the request object to a dictionary
