...
async def test_submit_success(overridden_get_db):
    ...
    2. Patch the `process_request` method of `PlantUserDeletePostModelResponse` with the mock function.
    3. Create a plant using the `PlantFactory`.
    4. Generate an API key for the plant.
    5. Send a POST request to the `plant-user-delete` endpoint with the plant code and API key.
...
async def test_submit_request_validation_error(overridden_get_db):
    ...
    3. Send a POST request to the `plant-user-delete` endpoint with the plant code, invalid data, and API key.
