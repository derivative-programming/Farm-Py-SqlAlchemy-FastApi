...
from .....models.factory.plant_user_details  import RequestFactory
...
# @pytest.fixture(scope="module")
# def valid_header(plant):
#     # Replace with your actual token generation logic
#     api_dict = {'PlantCode': str(plant.code), 'role_name_csv': 'User'}
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/plant-user-details/{plant.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
# @pytest.mark.asyncio
# async def test_submit_failure(overridden_get_db, invalid_request_data, valid_header, plant):
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/plant-user-details/{plant.code}/',
...
#             '/api/v1_0/plant-user-details/xxx/',
#             headers=valid_header
#         )
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/plant-user-details/{plant_code}/initxxx/'
        )
        assert response.status_code == 501
