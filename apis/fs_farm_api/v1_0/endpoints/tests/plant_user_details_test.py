...
async def test_init_authorization_failure_bad_api_key(overridden_get_db: AsyncSession):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        if PlantUserDetailsRouterConfig.is_public == True:
            assert response.status_code == 200
        else:
...
async def test_get_success(overridden_get_db: AsyncSession, api_key_fixture:str):
    async def mock_process_request(session, session_context, plant_code, request):
        pass
    with patch.object(apis_models.PlantUserDetailsGetModelResponse, 'process_request', new_callable=AsyncMock) as mock_method:
