...
from .....models.factory.plant_user_property_random_update  import PlantUserPropertyRandomUpdatePostModelRequestFactory
...
#     async with AsyncClient(app=app, base_url="http://test") as ac:
#         response = await ac.post(
#             f'/api/v1_0/plant-user-property-random-update/{.code}/',
#             json=valid_request_data,
#             headers=valid_header
#         )
...
#             '/api/v1_0/plant-user-property-random-update/xxx/',
#             headers=valid_header
#         )
...
async def test_init_success(overridden_get_db: AsyncSession):
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        response = await ac.get(
            f'/api/v1_0/plant-user-property-random-update/{plant_code}/init',
...
async def test_init_failure():
    ...
    async with AsyncClient(app=app, base_url="http://test") as ac:
        response = await ac.get(
            '/api/v1_0/plant-user-property-random-update/{plant_code}/initxxx/'
        )
        assert response.status_code == 501
