...
"""
This module contains unit tests for the `tac_farm_dashboard` endpoint.
The `tac_farm_dashboard` endpoint is responsible for handling requests related to
...
async def test_get_success(
    ...
    """
    Test the successful retrieval of the tac plant list.
    """
...
async def test_get_authorization_failure_no_header(
    ...
    '/api/v1_0/tac-farm-dashboard/{tac_code}' endpoint
...
async def test_get_endpoint_invalid_code_failure(
    ...
    """
    Test case for the GET endpoint when an invalid tac code is provided.
    Args:
        overridden_get_db (AsyncSession): The overridden database session.
        api_key_fixture (str): The API key fixture.
    Returns:
...
async def test_get_csv_authorization_failure_bad_api_key(
    ...
    2. Create a TacFarmDashboardGetModelRequest using
        the TacFarmDashboardGetModelRequestFactory.
    3. Convert the request object to a dictionary.
    4. Send a GET request to the API endpoint with
        the tac code and request parameters.
...
async def test_get_csv_authorization_failure_empty_header_key(
    ...
    If the TacFarmDashboardRouterConfig.is_public is True:
    - The response status code should be 200.
    - The response content type should start with the test_constants.
        REPORT_TO_CSV_MEDIA_TYPE.
    If the TacFarmDashboardRouterConfig.is_public is False:
...
async def test_get_csv_endpoint_url_failure(
    ...
    This test case verifies that the API endpoint
        '/api/v1_0/tac-farm-dashboard/{tac_code}/to-csv/xxx'
...
async def test_get_csv_endpoint_invalid_code_failure(
    ...
    2. Create a request object using the TacFarmDashboardGetModelRequestFactory.
    3. Convert the request object to a dictionary
