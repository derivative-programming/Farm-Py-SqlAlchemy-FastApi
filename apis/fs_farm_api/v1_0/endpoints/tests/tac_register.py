...
from apis.factories import TacRegisterPostModelRequestFactory
...
class TacRegisterViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        logging.info('call TacRegisterPostModelRequestFactory.create()')
        self.request = TacRegisterPostModelRequestFactory.create()
    ...
    def test_post_not_implemented(self):
        ...
        logging.info('/api/v1_0/tac-register/')
        logging.info(self.valid_request_data)
        response = self.client.post('/api/v1_0/tac-register/', self.valid_request_data, content_type='application/json')
        self.assertEqual(response.status_code, 501)

    def test_submit_success(self):
        # Assuming you have a FlowTacRegister.process method that handles valid data
        logging.info(f'/api/v1_0/tac-register/{self.tac.code}/')
        logging.info(self.valid_request_data)
    ...
    def test_submit_failure(self):
        ...
        response = self.client.post(f'/api/v1_0/tac-register/{self.tac.code}/submit/', data=self.invalid_request_data, **self.valid_header, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        json_string = response.content.decode()
        responseDict = json.loads(json_string)
