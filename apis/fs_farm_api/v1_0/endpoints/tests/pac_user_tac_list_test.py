# apis/fs_farm_api/v1_0/endpoints/tests/pac_user_tac_list_test.py
# pylint: disable=unused-import
"""
This module contains unit tests for the `pac_user_tac_list` endpoint.
The `pac_user_tac_list` endpoint is responsible for handling requests related to
...
async def test_get_success(
    ...
    async def mock_process_request(
        ...
        pass
    with patch.object(
        apis_models.PacUserTacListGetModelResponse,
        'process_request',
...
async def test_get_authorization_failure_no_header(
    ...
    '/api/v1_0/pac-user-tac-list/{pac_code}' endpoint
...
async def test_get_endpoint_url_failure(
    ...
    pac_code = pac.code
    request = await (
        request_factory.PacUserTacListGetModelRequestFactory.
        create_async(
            overridden_get_db
        )
    )
...
async def test_get_csv_authorization_failure_bad_api_key(
    ...
    This test case sends a GET request to the
    '/api/v1_0/pac-user-tac-list/{pac_code}/to-csv' endpoint
    ...
    2. Create a PacUserTacListGetModelRequest using
        the PacUserTacListGetModelRequestFactory.
...
async def test_get_csv_authorization_failure_empty_header_key(
    ...
    Steps:
    1. Create a test pac using the PacFactory.
    2. Create a PacUserTacListGetModelRequest using the request_factory.
    ...
    If the PacUserTacListRouterConfig.is_public is True:
    - The response status code should be 200.
    - The response content type should start with the test_constants.
        REPORT_TO_CSV_MEDIA_TYPE.
    If the PacUserTacListRouterConfig.is_public is False:
...
async def test_get_csv_authorization_failure_no_header(
    ...
    on the configuration.
    If the 'is_public' flag in the PacUserTacListRouterConfig is True:
    - The response status code should be 200.
    - The response content type should start with the
        'REPORT_TO_CSV_MEDIA_TYPE' defined in the test_constants.
    If the 'is_public' flag in the PacUserTacListRouterConfig is False:
    - The response status code should be 401.
    """
    pac = await model_factorys.PacFactory.create_async(overridden_get_db)
    pac_code = pac.code
...
async def test_get_csv_endpoint_url_failure(
    ...
    This test case verifies that the API endpoint
        '/api/v1_0/pac-user-tac-list/{pac_code}/to-csv/xxx'
...
async def test_get_csv_endpoint_invalid_code_failure(
    ...
    2. Create a request object using the PacUserTacListGetModelRequestFactory.
    3. Convert the request object to a dictionary
