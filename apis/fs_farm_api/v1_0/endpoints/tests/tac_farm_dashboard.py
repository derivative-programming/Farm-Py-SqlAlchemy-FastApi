...
class TacFarmDashboardViewSetTestCase(TestCase):
    ...
    def test_post_not_implemented(self):
        ...
        logging.debug('/api/v1_0/tac-farm-dashboard/')
        logging.debug(self.valid_request_data)
        response = self.client.post('/api/v1_0/tac-farm-dashboard/', self.valid_request_data, content_type='application/json')
        self.assertEqual(response.status_code, 501)

    def test_submit_success(self):
        # Assuming you have a FlowTacFarmDashboard.process method that handles valid data
        logging.debug(f'/api/v1_0/tac-farm-dashboard/{self.tac.code}/')
        logging.debug(self.valid_request_data)
        response = self.client.post(f'/api/v1_0/tac-farm-dashboard/{self.tac.code}/submit/', data=self.valid_request_data, **self.valid_header, format='json')
    ...
    def test_submit_failure(self):
        ...
        response = self.client.post(f'/api/v1_0/tac-farm-dashboard/{self.tac.code}/submit/', data=self.invalid_request_data, **self.valid_header, content_type='application/json')
        self.assertEqual(response.status_code, 200)
        json_string = response.content.decode()
        responseDict = json.loads(json_string)
