# apis/fs_farm_api/v1_0/endpoints/plant_user_details.py  # pylint: disable=duplicate-code # noqa: E501
...
class PlantUserDetailsRouterConfig():  # pylint: disable=too-few-public-methods
    """
...
class PlantUserDetailsRouter(BaseRouter):
    ...
    async def request_get_init(
        ...
        logging.info(
            "PlantUserDetailsRouter"
            ".request_get_init start. plantCode:%s",
            plant_code)
        ...
        async with session:
            try:
                ...
                logging.info(
                    "PlantUserDetailsRouter.request_get_init"
    ...
    async def request_get_with_id(
        plant_code: uuid.UUID = Path(..., description=PLANT_CODE),
        request_model:
            api_models.PlantUserDetailsGetModelRequest = (
                Depends()),
        session: AsyncSession = Depends(get_db),
        api_key: str = Depends(api_key_header)
    ):
        ...
        logging.info(
            "PlantUserDetailsRouter"
            ".request_get_with_id start. plantCode:%s",
            plant_code)
        auth_dict = BaseRouter.implementation_check(
            PlantUserDetailsRouterConfig
    ...
    async def request_get_with_id_to_csv(
        ...
        async with session:
            try:
                ...
                report_manager = \
                    reports.ReportManagerPlantUserDetails(
                        session_context)
        ...
        output_file_name = (
            "plant_user_details_"
            f"{str(plant_code)}_{str(uuid.uuid4())}.csv"
        )
        return FileResponse(
            tmp_file_path,
            media_type='text/csv',
