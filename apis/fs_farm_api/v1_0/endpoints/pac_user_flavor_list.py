...
class PacUserFlavorListRouter():
    ...
    async def request_get_init(pac_code: str, session:AsyncSession = Depends(get_db), api_key: str = Depends(api_key_header)):
        ...
        logging.info('PacUserFlavorListRouter.init get result:' + response.to_json())
        return response
        ...
        async with session:
            try:
                ...
                if response.success == True:
                    ...
                    await session.rollback()
        logging.info('PacUserFlavorListRouter.submit get result:' + response.to_json())
        return response

    @staticmethod
    @router.get("/api/v1_0/pac-user-flavor-list/{pac_code}/to-csv", response_model=api_models.PacUserFlavorListGetModelResponse)
    async def request_get_with_id_to_csv(pac_code: str, request_model:api_models.PacUserFlavorListGetModelRequest, session:AsyncSession = Depends(get_db), api_key: str = Depends(api_key_header)):
