...
The PacUserTriStateFilterListRouter provides
the following endpoints:
- GET /api/v1_0/pac-user-tri-state-filter-list/...
    {pac_code}/init:
...
class PacUserTriStateFilterListRouter(BaseRouter):
    ...
    async def request_get_with_id(
        pac_code: uuid.UUID = Path(..., description=PAC_CODE),
        request_model:
            api_models.PacUserTriStateFilterListGetModelRequest = (
                Depends()),
        session: AsyncSession = Depends(get_db),
        api_key: str = Depends(api_key_header)
    ):
        ...
        Returns:
            api_models.PacUserTriStateFilterListGetModelResponse:
                The response
        ...
        async with session:
            try:
                ...
                logging.info(
                    'PacUserTriStateFilterListRouter success')
            except Exception as e:  # pylint: disable=broad-exception-caught
                logging.info(
                    EXCEPTION_OCCURRED,
                    e.__class__.__name__,
        ...
        response_data = response.model_dump_json()
        logging.info(
            "PacUserTriStateFilterListRouter.submit get result:%s",
            response_data
        )
        return response
    ...
    async def request_get_with_id_to_csv(
        ...
        async with session:
            try:
                ...
                report_manager = \
                    reports.ReportManagerPacUserTriStateFilterList(
                        session_context)
        ...
        output_file_name = (
            "pac_user_tri_state_filter_list_"
