...
class PacUserTriStateFilterListRouter(BaseRouter):
        ...
        async with session:
            try:
                ...
                if response.success is True:
                    ...
                    await session.rollback()
        logging.info('PacUserTriStateFilterListRouter.submit get result:$s', response.model_dump_json())
        return response
    ...
    async def request_get_with_id_to_csv(
        ...
        logging.info(
            'PacUserTriStateFilterListRouter.request_get_with_id_to_csv start. pacCode:%s',
             pac_code)
        auth_dict = BaseRouter.implementation_check(
