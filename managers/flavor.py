# models/managers/flavor.py  # pylint: disable=duplicate-code # noqa: E501
...
class FlavorManager:
    ...
    async def from_enum(
        ...
        """
        query_filter = (
            Flavor._lookup_enum_name == enum_val.value)  # pylint: disable=protected-access  # noqa: E501
    ...
    def _build_query(self):
        ...
        query = query.outerjoin(  # pac_id
            Pac,
            and_(Flavor._pac_id == Pac._pac_id,  # type: ignore
                 Flavor._pac_id != 0)  # type: ignore
        )
    ...
    async def get_by_code(
        ...
        Returns:
            Optional[Flavor]: The retrieved
                flavor, or None if not found.
        """
        logging.info("FlavorManager.get_by_code %s",
                     code)
    ...
    async def delete(self, flavor_id: int):
        ...
        Raises:
            TypeError: If the flavor_id
                is not an integer.
            FlavorNotFoundError: If the
                flavor with the
                specified ID is not found.
        """
        logging.info(
            "FlavorManager.delete %s",
            flavor_id)
        if not isinstance(flavor_id, int):
            raise TypeError(
    ...
    async def from_json(self, json_str: str) -> Flavor:
        """
        ...
        if new_flavor is None:
            ...
            for key, value in flavor_dict.items():
                setattr(new_flavor, key, value)

        return new_flavor

    async def from_dict(
        self, flavor_dict: Dict[str, Any]
    ) -> Flavor:
        """
        Creates a Flavor
