...
async def test_add(land_manager):
    ...
    assert added_land
    assert added_land.id
@pytest.mark.asyncio
async def test_get_by_id(land_manager):
    land_data = factory.build(dict, FACTORY_CLASS=LandFactory)
    added_land = await land_manager.add(**land_data)
    fetched_land = await land_manager.get_by_id(added_land.id)
    assert fetched_land.id == added_land.id
@pytest.mark.asyncio
...
async def test_update(land_manager):
    ...
    await land_manager.update(added_land.id, code=new_code)
    fetched_land = await land_manager.get_by_id(added_land.id)
...
async def test_delete(land_manager):
    land_data = factory.build(dict, FACTORY_CLASS=LandFactory)
    added_land = await land_manager.add(**land_data)
    await land_manager.delete(added_land.id)
    fetched_land = await land_manager.get_by_id(added_land.id)
