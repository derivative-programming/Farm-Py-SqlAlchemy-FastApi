...
async def test_add(role_manager):
    ...
    assert added_role
    assert added_role.id
@pytest.mark.asyncio
async def test_get_by_id(role_manager):
    role_data = factory.build(dict, FACTORY_CLASS=RoleFactory)
    added_role = await role_manager.add(**role_data)
    fetched_role = await role_manager.get_by_id(added_role.id)
    assert fetched_role.id == added_role.id
@pytest.mark.asyncio
...
async def test_update(role_manager):
    ...
    await role_manager.update(added_role.id, code=new_code)
    fetched_role = await role_manager.get_by_id(added_role.id)
...
async def test_delete(role_manager):
    role_data = factory.build(dict, FACTORY_CLASS=RoleFactory)
    added_role = await role_manager.add(**role_data)
    await role_manager.delete(added_role.id)
    fetched_role = await role_manager.get_by_id(added_role.id)
