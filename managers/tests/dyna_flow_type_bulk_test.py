# models/managers/tests/dyna_flow_type_test.py  # pylint: disable=duplicate-code
...
class TestDynaFlowTypeBulkManager:
    """
    This class contains unit tests for the
    `DynaFlowTypeManager` class.
    """
    ...
    async def test_add_bulk(
        ...
        dyna_flow_types to the database.

        Steps:
        1. Generate a list of dyna_flow_type data using the
            `DynaFlowTypeFactory.build_async` method.
        2. Call the `add_bulk` method of the
            `obj_manager` instance,
            passing in the
            generated dyna_flow_type data.
        3. Verify that the number of dyna_flow_types
            returned is
            equal to the number of dyna_flow_types added.
        4. For each updated dyna_flow_type, fetch the corresponding
            dyna_flow_type from the database.
        5. Verify that the fetched dyna_flow_type
            is an instance of the
            `DynaFlowType` class.
        6. Verify that the insert_user_id and
            last_update_user_id of the fetched
            dyna_flow_type match the
            customer code of the session context.
        7. Verify that the dyna_flow_type_id of the fetched
            dyna_flow_type matches the
            dyna_flow_type_id of the updated
            dyna_flow_type.

        """
        dyna_flow_types_data = [
            await DynaFlowTypeFactory.build_async(session)
            for _ in range(5)]

        dyna_flow_types = await obj_manager.add_bulk(
            dyna_flow_types_data)

        assert len(dyna_flow_types) == 5

        for updated_obj in dyna_flow_types:
            result = await session.execute(
                select(DynaFlowType).filter(
                    DynaFlowType._dyna_flow_type_id == (
                        updated_obj.dyna_flow_type_id)  # type: ignore
                )
            )
    ...
    async def test_update_bulk_success(
        ...
        """
        Test case for bulk update of dyna_flow_types.

        This test case verifies the functionality of the
        `update_bulk` method in the
        `DynaFlowTypeManager` class.
        It creates two dyna_flow_type instances,
        updates their codes
        ...
        Steps:
        1. Create two dyna_flow_type instances using the
            `DynaFlowTypeFactory.create_async` method.
        2. Generate new codes for the dyna_flow_types.
        3. Update the dyna_flow_types' codes
        ...
        Returns:
            None
        """
        # Mocking dyna_flow_type instances
        obj_1 = await DynaFlowTypeFactory. \
            create_async(
                session=session)
        obj_2 = await DynaFlowTypeFactory. \
            create_async(
                session=session)
        ...
        updates = [
            ...
            {
                "dyna_flow_type_id":
                    obj_2.dyna_flow_type_id,
                "code": code_updated2
            }
        ]
        updated_dyna_flow_types = await obj_manager.update_bulk(
            updates)

        logging.info('bulk update results')
        # Assertions
        assert len(updated_dyna_flow_types) == 2
        logging.info(updated_dyna_flow_types[0]
                     .__dict__)
        logging.info(updated_dyna_flow_types[1]
                     .__dict__)

        logging.info('getall')
        dyna_flow_types = await obj_manager.get_list()
        logging.info(dyna_flow_types[0]
                     .__dict__)
        logging.info(dyna_flow_types[1]
                     .__dict__)

        assert updated_dyna_flow_types[0].code == \
            code_updated1
        assert updated_dyna_flow_types[1].code == \
            code_updated2

        assert str(updated_dyna_flow_types[0]
                   .last_update_user_id) == (
            str(obj_manager
                ._session_context.customer_code))

        assert str(updated_dyna_flow_types[1]
                   .last_update_user_id) == (
            str(obj_manager
                ._session_context.customer_code))
        ...
        result = await session.execute(
            select(DynaFlowType).filter(
                DynaFlowType._dyna_flow_type_id == 1)  # type: ignore
        )
        fetched_obj = result.scalars().first()
        ...
        result = await session.execute(
            select(DynaFlowType).filter(
                DynaFlowType._dyna_flow_type_id == 2)  # type: ignore
        )
        fetched_obj = result.scalars().first()
    ...
    @pytest.mark.asyncio
    async def test_update_bulk_missing_dyna_flow_type_id(
        self,
        obj_manager: DynaFlowTypeManager,
        session: AsyncSession
    ):
        ...
        method when the dyna_flow_type_id is missing.

        This test case ensures that when the dyna_flow_type_id is
        missing in the updates list,
        an exception is raised and the session is rolled back.

        Steps:
        1. Prepare the updates list with a missing dyna_flow_type_id.
        ...
        """
        # No dyna_flow_types to update since
        # dyna_flow_type_id is missing
    ...
    @pytest.mark.asyncio
    async def test_update_bulk_dyna_flow_type_not_found(
        self,
        obj_manager: DynaFlowTypeManager,
        session: AsyncSession
    ):
        """
        Test case to verify the behavior of the update_bulk
        method when a dyna_flow_type is not found.

        This test case performs the following steps:
        1. Defines a list of dyna_flow_type updates,
            where each update
            contains a dyna_flow_type_id and a code.
        ...
        3. Expects an exception to be raised, indicating that
            the dyna_flow_type was not found.
        ...
        # Update dyna_flow_types
        updates = [{"dyna_flow_type_id": 1, "code": uuid.uuid4()}]
    ...
    async def test_update_bulk_invalid_type(
        ...
        updates = [{"dyna_flow_type_id": "2", "code": uuid.uuid4()}]
    ...
    async def test_delete_bulk_success(
        ...
        obj_2 = await DynaFlowTypeFactory.create_async(
            session=session)

        # Delete dyna_flow_types
        dyna_flow_type_ids = [
            obj_1.dyna_flow_type_id,
            obj_2.dyna_flow_type_id
        ]
    ...
    @pytest.mark.asyncio
    async def test_delete_bulk_dyna_flow_types_not_found(
        self,
        obj_manager: DynaFlowTypeManager,
        session: AsyncSession
    ):
        ...
        dyna_flow_types when some
        dyna_flow_types are not found.

        Steps:
        1. Create a dyna_flow_type using the
            DynaFlowTypeFactory.
        2. Assert that the created dyna_flow_type
            is an instance of the
            DynaFlowType class.
        3. Define a list of dyna_flow_type IDs to delete.
        4. Use pytest.raises to assert that an exception is
            raised when deleting the bulk dyna_flow_types.
        5. Rollback the session to undo any changes made during the test.

        This test case ensures that the delete_bulk method of the
        DynaFlowTypeManager raises an exception
        when some dyna_flow_types with the specified IDs are
    ...
    async def test_delete_bulk_empty_list(
        ...
        dyna_flow_types with an empty list.

        Args:
            obj_manager (DynaFlowTypeManager): The
                instance of the
                DynaFlowTypeManager class.
        ...
        # Delete dyna_flow_types with an empty list
        dyna_flow_type_ids = []
        result = await obj_manager.delete_bulk(
            dyna_flow_type_ids)
    ...
    async def test_delete_bulk_invalid_type(
        ...
        method when invalid dyna_flow_type IDs are provided.
