...
async def test_add(flavor_manager):
    ...
    assert added_flavor
    assert added_flavor.id
@pytest.mark.asyncio
async def test_get_by_id(flavor_manager):
    flavor_data = factory.build(dict, FACTORY_CLASS=FlavorFactory)
    added_flavor = await flavor_manager.add(**flavor_data)
    fetched_flavor = await flavor_manager.get_by_id(added_flavor.id)
    assert fetched_flavor.id == added_flavor.id
@pytest.mark.asyncio
...
async def test_update(flavor_manager):
    ...
    await flavor_manager.update(added_flavor.id, code=new_code)
    fetched_flavor = await flavor_manager.get_by_id(added_flavor.id)
...
async def test_delete(flavor_manager):
    flavor_data = factory.build(dict, FACTORY_CLASS=FlavorFactory)
    added_flavor = await flavor_manager.add(**flavor_data)
    await flavor_manager.delete(added_flavor.id)
    fetched_flavor = await flavor_manager.get_by_id(added_flavor.id)
