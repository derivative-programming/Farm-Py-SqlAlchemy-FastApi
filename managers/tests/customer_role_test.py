...
async def test_add(customer_role_manager):
    ...
    assert added_customer_role.id
@pytest.mark.asyncio
async def test_get_by_id(customer_role_manager):
    customer_role_data = factory.build(dict, FACTORY_CLASS=CustomerRoleFactory)
    added_customer_role = await customer_role_manager.add(**customer_role_data)
    fetched_customer_role = await customer_role_manager.get_by_id(added_customer_role.id)
    assert fetched_customer_role.id == added_customer_role.id
@pytest.mark.asyncio
...
async def test_update(customer_role_manager):
    ...
    await customer_role_manager.update(added_customer_role.id, code=new_code)
    fetched_customer_role = await customer_role_manager.get_by_id(added_customer_role.id)
...
async def test_delete(customer_role_manager):
    customer_role_data = factory.build(dict, FACTORY_CLASS=CustomerRoleFactory)
    added_customer_role = await customer_role_manager.add(**customer_role_data)
    await customer_role_manager.delete(added_customer_role.id)
    fetched_customer_role = await customer_role_manager.get_by_id(added_customer_role.id)
