...
async def test_add(org_customer_manager):
    ...
    assert added_org_customer.id
@pytest.mark.asyncio
async def test_get_by_id(org_customer_manager):
    org_customer_data = factory.build(dict, FACTORY_CLASS=OrgCustomerFactory)
    added_org_customer = await org_customer_manager.add(**org_customer_data)
    fetched_org_customer = await org_customer_manager.get_by_id(added_org_customer.id)
    assert fetched_org_customer.id == added_org_customer.id
@pytest.mark.asyncio
...
async def test_update(org_customer_manager):
    ...
    await org_customer_manager.update(added_org_customer.id, code=new_code)
    fetched_org_customer = await org_customer_manager.get_by_id(added_org_customer.id)
...
async def test_delete(org_customer_manager):
    org_customer_data = factory.build(dict, FACTORY_CLASS=OrgCustomerFactory)
    added_org_customer = await org_customer_manager.add(**org_customer_data)
    await org_customer_manager.delete(added_org_customer.id)
    fetched_org_customer = await org_customer_manager.get_by_id(added_org_customer.id)
