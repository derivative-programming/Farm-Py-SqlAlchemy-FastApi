...
async def test_add(tri_state_filter_manager):
    ...
    assert added_tri_state_filter.id
@pytest.mark.asyncio
async def test_get_by_id(tri_state_filter_manager):
    tri_state_filter_data = factory.build(dict, FACTORY_CLASS=TriStateFilterFactory)
    added_tri_state_filter = await tri_state_filter_manager.add(**tri_state_filter_data)
    fetched_tri_state_filter = await tri_state_filter_manager.get_by_id(added_tri_state_filter.id)
    assert fetched_tri_state_filter.id == added_tri_state_filter.id
@pytest.mark.asyncio
...
async def test_update(tri_state_filter_manager):
    ...
    await tri_state_filter_manager.update(added_tri_state_filter.id, code=new_code)
    fetched_tri_state_filter = await tri_state_filter_manager.get_by_id(added_tri_state_filter.id)
...
async def test_delete(tri_state_filter_manager):
    tri_state_filter_data = factory.build(dict, FACTORY_CLASS=TriStateFilterFactory)
    added_tri_state_filter = await tri_state_filter_manager.add(**tri_state_filter_data)
    await tri_state_filter_manager.delete(added_tri_state_filter.id)
    fetched_tri_state_filter = await tri_state_filter_manager.get_by_id(added_tri_state_filter.id)
