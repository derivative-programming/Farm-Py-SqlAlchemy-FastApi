...
async def test_add(organization_manager):
    ...
    assert added_organization.id
@pytest.mark.asyncio
async def test_get_by_id(organization_manager):
    organization_data = factory.build(dict, FACTORY_CLASS=OrganizationFactory)
    added_organization = await organization_manager.add(**organization_data)
    fetched_organization = await organization_manager.get_by_id(added_organization.id)
    assert fetched_organization.id == added_organization.id
@pytest.mark.asyncio
...
async def test_update(organization_manager):
    ...
    await organization_manager.update(added_organization.id, code=new_code)
    fetched_organization = await organization_manager.get_by_id(added_organization.id)
...
async def test_delete(organization_manager):
    organization_data = factory.build(dict, FACTORY_CLASS=OrganizationFactory)
    added_organization = await organization_manager.add(**organization_data)
    await organization_manager.delete(added_organization.id)
    fetched_organization = await organization_manager.get_by_id(added_organization.id)
