...
@pytest.fixture
def land_base_bus_obj(
    fake_session_context, land
):
    """
    Fixture that returns a LandBaseBusObj instance.
    """
    land_base = LandBaseBusObj(
...
class TestLandBaseBusObj:
    ...
    async def test_create_land(
        ...
        assert isinstance(land_bus_obj.description,
                          str)
        assert isinstance(land_bus_obj.display_order,
                          int)
        assert isinstance(land_bus_obj.is_active,
                          bool)
        assert isinstance(land_bus_obj.lookup_enum_name,
                          str)
        assert isinstance(land_bus_obj.name,
                          str)
        assert isinstance(land_bus_obj.pac_id,
                          int)
    ...
    async def test_refresh(
        ...
        with patch(
            ...
            mock_land_manager_instance = \
                mock_land_manager.return_value
            mock_land_manager_instance.refresh =\
