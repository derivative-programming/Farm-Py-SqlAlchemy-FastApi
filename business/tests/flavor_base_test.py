...
@pytest.fixture
def flavor_base_bus_obj(
    fake_session_context, flavor
):
    """
    Fixture that returns a FlavorBaseBusObj instance.
    """
    flavor_base = FlavorBaseBusObj(
...
class TestFlavorBaseBusObj:
    ...
    async def test_create_flavor(
        ...
        assert isinstance(flavor_bus_obj.description,
                          str)
        assert isinstance(flavor_bus_obj.display_order,
                          int)
        assert isinstance(flavor_bus_obj.is_active,
                          bool)
        assert isinstance(flavor_bus_obj.lookup_enum_name,
                          str)
        assert isinstance(flavor_bus_obj.name,
                          str)
        assert isinstance(flavor_bus_obj.pac_id,
                          int)
    ...
    async def test_delete_flavor(
        ...
        assert flavor_bus_obj.flavor is not None
    ...
    async def test_refresh(
        self, flavor_base_bus_obj, flavor):
        """
        ...
        with patch(
            ...
            mock_flavor_manager_instance = \
                mock_flavor_manager.return_value
            mock_flavor_manager_instance.refresh =\
    ...
    def test_last_change_code(
        ...
        Args:
            ...
            flavor (Flavor): An instance of the
                Flavor class.
