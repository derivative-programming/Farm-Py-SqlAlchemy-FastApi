...
class ReportManagerPacUserDateGreaterThanFilterList():
    ...
    def __init__(self, session_context: SessionContext):
        ...
        if session_context.session is None:
            raise TypeError("ReportManagerPacUserDateGreaterThanFilterList.init session_context has no session assigned")
    async def generate(self,
        ...
        return result
    async def build_csv(self, file_name: str, data_list: List[ReportItemPacUserDateGreaterThanFilterList]):
        with open(file_name, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=vars(ReportItemPacUserDateGreaterThanFilterList()).keys(), quoting=csv.QUOTE_ALL)
            writer.writeheader()
