...
class ReportManagerPacUserRoleList():
    ...
    def __init__(self, session_context: SessionContext):
        ...
        if session_context.session is None:
            raise TypeError(
                "ReportManagerPacUserRoleList.init session_context has "
                "no session assigned"
            )
    async def generate(
        self,
...
# endset  # noqa: E122
        ...
        for data_item in data_list:
            report_item: ReportItemPacUserRoleList = ReportItemPacUserRoleList()
    ...
    async def build_csv(self,
        ...
        with open(file_name, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(
                file,
                fieldnames=vars(ReportItemPacUserRoleList()).keys(),
