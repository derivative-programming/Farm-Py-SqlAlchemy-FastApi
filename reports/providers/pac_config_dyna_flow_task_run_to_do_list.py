# reports/providers/pac_config_dyna_flow_task_run_to_do_list.py  # pylint: disable=duplicate-code # noqa: E501
...
class ReportProviderPacConfigDynaFlowTaskRunToDoList():
    """
    The ReportProviderPacConfigDynaFlowTaskRunToDoList
    ...
    async def generate_list(
        ...
        query_dict["context_code"] = (
            str(context_code))
        query_dict["is_run_task_debug_required_tri_state_filter_code"] = (
            str(is_run_task_debug_required_tri_state_filter_code))
        query_dict["like_is_run_task_debug_required_tri_state_filter_code"] = (
            str(is_run_task_debug_required_tri_state_filter_code))
        ...
        if ReportProviderPacConfigDynaFlowTaskRunToDoList \
                ._cached_sql_query == "":
            ...
            if 'postgresql' in db_engine_url:
                sql_folder = "postgres"
            # Prioritize
            # 'pac_config_dyna_flow_task_run_to_do_list.inc.sql'
            # if it exists
            inc_file_path = (
            ...
            gen_file_path = (
                f"reports/providers/sql/{sql_folder}/"
                "pac_config_dyna_flow_task_run_to_do_list.gen.sql"
            )
        ...
        cursor = await self._session_context.session.execute(
            text(ReportProviderPacConfigDynaFlowTaskRunToDoList
                 ._cached_sql_query),
            query_dict
        )
