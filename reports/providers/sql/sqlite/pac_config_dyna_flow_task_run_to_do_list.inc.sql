	...
	(
		...
		where
			...
			and (
				...
				:is_run_task_debug_required_tri_state_filter_code = REPLACE('00000000-0000-0000-0000-000000000000', '-', '') or
				(
					(
						select
							state_int_value
						...
						where code = REPLACE(:is_run_task_debug_required_tri_state_filter_code, '-', '')
					) in (
					-1,
		...
		((ifnull(dep_task.is_completed,1) = 1 and ifnull(dep_task.is_successful,1) = 1) or ifnull(dep_task.is_canceled,1) = 1) and
		((ifnull(dep_flow.is_completed,1) = 1 and ifnull(dep_flow.is_successful,1) = 1) or ifnull(dep_flow.is_canceled,1) = 1) and 
