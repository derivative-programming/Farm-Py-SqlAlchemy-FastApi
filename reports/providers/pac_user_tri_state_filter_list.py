# reports/providers/pac_user_tri_state_filter_list.py  # pylint: disable=duplicate-code # noqa: E501
...
class ReportProviderPacUserTriStateFilterList():
    ...
    async def generate_list(
        ...
        if ReportProviderPacUserTriStateFilterList \
                ._cached_sql_query == "":
            ...
            if 'postgresql' in db_engine_url:
                sql_folder = "postgres"
            # Prioritize
            # 'pac_user_tri_state_filter_list.inc.sql'
            # if it exists
            inc_file_path = (
            ...
            gen_file_path = (
                f"reports/providers/sql/{sql_folder}/"
                "pac_user_tri_state_filter_list.gen.sql"
            )
        ...
        cursor = await self._session_context.session.execute(
            text(ReportProviderPacUserTriStateFilterList
                 ._cached_sql_query),
            query_dict
        )
