# reports/providers/pac_config_dyna_flow_dft_build_to_do_list.py  # pylint: disable=duplicate-code # noqa: E501
...
of pac dyna_flows based on the provided parameters.
"""
...
class ReportProviderPacConfigDynaFlowDFTBuildToDoList():
    """
    The ReportProviderPacConfigDynaFlowDFTBuildToDoList
    class is responsible
    for generating a list of pac dyna_flows
    ...
    async def generate_list(
        ...
        """
        Generates a list of pac dyna_flows based
        on the provided parameters.

        Returns:
            list[dict[str, Any]]: The list of
            pac dyna_flows as dictionaries.
        """
        flow_name = (
        ...
        query_dict["context_code"] = (
            str(context_code))
        query_dict["is_build_task_debug_required_tri_state_filter_code"] = (
            str(is_build_task_debug_required_tri_state_filter_code))
        query_dict["like_is_build_task_debug_required_tri_state_filter_code"] = (
            str(is_build_task_debug_required_tri_state_filter_code))
        ...
        cursor = await self._session_context.session.execute(
            text(ReportProviderPacConfigDynaFlowDFTBuildToDoList
                 ._cached_sql_query),
            query_dict
        )
