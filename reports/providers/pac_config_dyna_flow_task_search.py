# reports/providers/pac_config_dyna_flow_task_search.py  # pylint: disable=duplicate-code # noqa: E501
...
class ReportProviderPacConfigDynaFlowTaskSearch():
    """
    The ReportProviderPacConfigDynaFlowTaskSearch
    ...
    async def generate_list(
        ...
        query_dict["context_code"] = (
            str(context_code))
        query_dict["started_date_greater_than_filter_code"] = (
            str(started_date_greater_than_filter_code))
        query_dict["processor_identifier"] = (
            processor_identifier)
        query_dict["is_started_tri_state_filter_code"] = (
            str(is_started_tri_state_filter_code))
        query_dict["is_completed_tri_state_filter_code"] = (
            str(is_completed_tri_state_filter_code))
        query_dict["is_successful_tri_state_filter_code"] = (
            str(is_successful_tri_state_filter_code))
        query_dict["like_started_date_greater_than_filter_code"] = (
            str(started_date_greater_than_filter_code))
        query_dict["like_processor_identifier"] = (
            '%' + processor_identifier + '%')
        query_dict["like_is_started_tri_state_filter_code"] = (
            str(is_started_tri_state_filter_code))
        query_dict["like_is_completed_tri_state_filter_code"] = (
            str(is_completed_tri_state_filter_code))
        query_dict["like_is_successful_tri_state_filter_code"] = (
            str(is_successful_tri_state_filter_code))
        ...
        cursor = await self._session_context.session.execute(
            text(ReportProviderPacConfigDynaFlowTaskSearch
                 ._cached_sql_query),
            query_dict
        )
