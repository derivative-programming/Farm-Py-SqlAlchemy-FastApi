# reports/providers/pac_config_dyna_flow_retry_task_build_list.py  # pylint: disable=duplicate-code # noqa: E501
...
class ReportProviderPacConfigDynaFlowRetryTaskBuildList():
    """
    The ReportProviderPacConfigDynaFlowRetryTaskBuildList
    ...
    async def generate_list(
        ...
        if ReportProviderPacConfigDynaFlowRetryTaskBuildList \
                ._cached_sql_query == "":
            ...
            if 'postgresql' in db_engine_url:
                sql_folder = "postgres"
            # Prioritize
            # 'pac_config_dyna_flow_retry_task_build_list.inc.sql'
            # if it exists
            inc_file_path = (
            ...
            gen_file_path = (
                f"reports/providers/sql/{sql_folder}/"
                "pac_config_dyna_flow_retry_task_build_list.gen.sql"
            )
        ...
        cursor = await self._session_context.session.execute(
            text(ReportProviderPacConfigDynaFlowRetryTaskBuildList
                 ._cached_sql_query),
            query_dict
        )
