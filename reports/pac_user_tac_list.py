...
class ReportManagerPacUserTacList():
    ...
    def __init__(self, session_context: SessionContext):
        ...
        if session_context.session is None:
            raise TypeError("ReportManagerPacUserTacList.init session_context has no session assigned")
    async def generate(self,
        ...
        return result
    async def build_csv(self, file_name: str, data_list: List[ReportItemPacUserTacList]):
        with open(file_name, mode='w', newline='', encoding='utf-8') as file:
            writer = csv.DictWriter(file, fieldnames=vars(ReportItemPacUserTacList()).keys(), quoting=csv.QUOTE_ALL)
            writer.writeheader()
