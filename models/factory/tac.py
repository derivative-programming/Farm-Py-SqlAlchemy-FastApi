...
import pytz
from models import Tac,TacSchema
...
class TacFactory(factory.Factory):
    ...
    def _create(cls, model_class, session, *args, **kwargs) -> Tac:
        ...
        return obj
    @classmethod
    async def create_async(cls, model_class, session, *args, **kwargs) -> Tac:
        pac_id_pac_instance = PacFactory.create(session=session)  #PacID
        ...
        return obj
    @classmethod
    async def build_async(cls, model_class, session, *args, **kwargs) -> Tac:
        pac_id_pac_instance = PacFactory.create(session=session)  #PacID
