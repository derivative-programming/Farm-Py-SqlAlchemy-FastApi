# models/dyna_flow_type.py  # pylint: disable=duplicate-code # noqa: E501
# pylint: disable=unused-import

"""
The DynaFlowType model inherits from
the Base model and is mapped to the
'farm_DynaFlowType' table in the database.
"""
...
from sqlalchemy import (BigInteger, Boolean,   # noqa: F401
                        ...
                        event, func)
import models.constants.dyna_flow_type as \
    dyna_flow_type_constants
from utils.common_functions import snake_case
from .base import Base, EncryptedType  # noqa: F401


class DynaFlowType(Base):
    """
    The DynaFlowType model represents a
    dyna_flow_type in the farm.
    It inherits from the Base model and is mapped to the
    'farm_DynaFlowType' table in the database.
    """

    __tablename__ = 'farm_' + snake_case('DynaFlowType')
    ...
    def code(self):
        """
        Get the code of the dyna_flow_type.

        Returns:
            UUID: The code of the dyna_flow_type.
        """
    ...
    def code(self, value: uuid.UUID):
        """
        Set the code of the dyna_flow_type.
    ...
    def dyna_flow_type_id(self) -> int:
        """
        Get the ID of the dyna_flow_type.

        Returns:
            int: The ID of the dyna_flow_type.
        """
    ...
    def pac_id(self) -> int:
        """
        Get the ID of the pac associated with this dyna_flow_type.
...
@event.listens_for(DynaFlowType, 'before_insert')
def set_created_on(
    mapper,
    connection,
    target
...
@event.listens_for(DynaFlowType, 'before_update')
def set_updated_on(
    mapper,
    connection,
    target
