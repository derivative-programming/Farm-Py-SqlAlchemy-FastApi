...
async def test_last_change_code_changes_on_update(session):
    # Create a tac and commit it to the database
    tac = TacFactory(session=session)
    # Store the initial last_change_code
    initial_last_change_code = tac.last_change_code
    # Update the code property of the tac
    tac.code = uuid.uuid4()  # Generating a new UUID for the code
    # Commit the update
    await session.commit()
    # Assert that the last_change_code has changed after the update
    assert tac.last_change_code != initial_last_change_code
    assert isinstance(tac.last_change_code, uuid.UUID)
@pytest.mark.asyncio
async def test_date_inserted(session):
    tac = TacFactory(session=session).build()
...
async def test_data_types(session):
    tac = TacFactory(session=session,some_int_val="12345", some_float_val="123.45")
    ...
    assert tac.other_flavor == "" or isinstance(tac.other_flavor, str)
    assert isinstance(tac.some_big_int_val, int)
    tac.some_varchar_val = "Changed"
    await session.commit()
    assert tac.last_update_utc_date_time > initial_time
@pytest.mark.asyncio
async def test_unique_code_constraint(session):
    tac_1 = TacFactory(session=session)
    tac_2 = TacFactory(session=session)
...
async def test_boolean_fields_default(session):
    ...
    assert isinstance(tac.some_utc_date_time_val, datetime.datetime)
    assert tac.some_varchar_val == "" or isinstance(tac.some_varchar_val, str)
...
async def test_last_change_code_concurrency(session):
    ...
    tac_1.some_varchar_val = "Change1"
    ...
    tac_2.some_varchar_val = "Change2"
...
async def test_invalid_flvr_foreign_key_id(session):
    tac = TacFactory(session=session,flvr_foreign_key_id=99999)  # Assume no Flavor with ID 99999
    session.add(tac)
    with pytest.raises(Exception):  # Adjust for the specific DB exception you'd expect
        await session.commit()
@pytest.mark.asyncio #PacID
async def test_invalid_pac_id(session):
    tac = TacFactory(session=session,pac_id=99999)  # Assume no Pac with ID 99999
    session.add(tac)
    with pytest.raises(Exception):  # Adjust for the specific DB exception you'd expect
        await session.commit()
