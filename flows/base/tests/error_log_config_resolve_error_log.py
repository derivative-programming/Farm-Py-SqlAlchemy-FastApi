...
from models.factories import ErrorLogFactory
...
class BaseFlowErrorLogConfigResolveErrorLogTestCase(unittest.TestCase):
    def setUp(self):
        CurrentRuntime.initialize()
        session_context = SessionContext(dict())
        self.flow = BaseFlowErrorLogConfigResolveErrorLog(session_context)
    ...
    def test_process_security_rules(self):
        error_log = ErrorLogFactory.create()
        # Call the method being tested
        self.flow._process_security_rules(error_log)
        # Add assertions here to validate the expected behavior
