# flows/constants/process_all_dyna_flow_type_schedule_task.py  # pylint: disable=duplicate-code # noqa: E501

"""
    This module contains configuration constants
    used in the process_all_dyna_flow_type_schedule_task flow.
"""

CALCULATED_IS_ROW_LEVEL_CUSTOMER_SECURITY_USED = False
CALCULATED_IS_ROW_LEVEL_ORG_CUSTOMER_SECURITY_USED = False
CALCULATED_IS_ROW_LEVEL_ORGANIZATION_SECURITY_USED = False
